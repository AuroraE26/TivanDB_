const DetalleOrden = require("../models/detalleOrden.model.js");
    ...
    DetalleOrden.create(detalleOrden, (err, data) => {
      if (err)
        res.status(500).send({
          code:500,
          message:
            err.message || "Error al dar de alta el detalle de orden."
        });
      else res.send(data);
    });
  };
    ...
    DetalleOrden.getAll ((err, data) => {
      if (err){
        res.status(500).send({
          code: 500,
          message:
            err.message || "No se obtuvieron los detalles de orden."
        })}
      else res.send(data);
      });
};
